...
typedef enum {CPY, INC, DEC, JNZ, TGL} Instructionset;
...
Instruction *do_tgl(Instruction *pc, int registers[4]);
...
void toggle(Instruction *inst, int new_instruction, ExecPtr new_exec_ptr) {
	inst->instruction = new_instruction;
	inst->do_instruction = new_exec_ptr;
}

Instruction *do_tgl(Instruction *pc, int registers[4]) {
	Instruction *t = pc;
	int steps = get_value(pc->c1, registers);
	int inc = steps < 0 ? 1 : -1;

	for(int i = steps; i != 0 && t != NULL; i += inc) {
		t = steps < 0 ? t->prev : t->next;
	}
...
int main(int argc, char **argv)
{
	...
	while( (buf = fgets(buf, 255, fp))) {
		...
		if(*buf == 't') {
			program = add_command(program, instruction_factory_create(TGL, buf + 4));
		}
	}
	fclose(fp);
